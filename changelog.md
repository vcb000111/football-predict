...
### Added (Thêm mới)
* **Hệ thống nhiều đoạn chat độc lập (Multi-Session Chats)**: Thiết lập bảng `chat_sessions` và cột `session_id` trong `assistant_chats` để hỗ trợ lưu trữ nhiều hội thoại độc lập cho cả người dùng đã đăng nhập (lưu database) và khách (lưu sessionStorage).
* **Tự động đặt tên cuộc trò chuyện bằng AI**: Tích hợp tính năng AI (Gemini) tự động phân tích câu hỏi đầu tiên của người dùng và đặt tên cho session ngắn gọn dạng Sentence case, cập nhật trực tiếp vào DB hoặc gửi về qua Server-Sent Events (SSE) để client cập nhật UI tức thời.
* **Tích hợp SweetAlert2 Dark Mode**: Thay thế toàn bộ các popup xác thực thô sơ của trình duyệt (`confirm()`, `alert()`) bằng thư viện SweetAlert2 được tùy biến phong cách tối (Dark mode) sang trọng, đồng bộ.
...
* **Phân giải Markdown & Bảng biểu trong Chatbox**: Tích hợp hàm `renderMessageContent` dùng chung từ module `markdown.js` để hiển thị tin nhắn AI rõ ràng, định dạng bảng biểu và chữ đậm chuẩn xác.
* **Lưu lịch sử Chatbox AI vào DB & Tải phân trang Infinite Scroll**: Tạo bảng `assistant_chats` (qua `migration_v3.mjs`), phát triển API `/api/chat/assistant/history` phân trang cursor-based (`beforeId`), cùng logic cuộn ngược trong widget Chatbox tự động bù trừ `scrollHeight` chống giật màn hình (Scroll Jump) và khóa `loadingHistory` chống spam request.
* **Upload nhiều ảnh lên Cloudinary & Nén client trong Chatbox**: Bổ sung chức năng đính kèm nhiều hình ảnh, nén ảnh bằng Canvas phía client để giảm tải payload và upload ảnh song song lên Cloudinary sử dụng cấu hình từ biến môi trường `CLOUDINARY_URL`.
...
* **Tích hợp bộ công cụ Function Calling cho AI Chatbox (10 Backend Tools)**: Xây dựng tệp tin [chat-tools.js](file:///d:/Projects/Football_Predict/src/lib/chat-tools.js) định nghĩa 10 công cụ (tìm kiếm tin tức, tra tỷ lệ kèo nhà cái, truy vấn DB lịch đấu, dự đoán, ELO tuyển, thống kê AI, chạy dự đoán realtime, cập nhật kết quả tự động, hot fixtures và current match context) cùng chuẩn khai báo schema tương thích với SDK `@google/genai` của Gemini.
* **Hệ thống gợi ý câu hỏi tiếp theo phía Client (Dynamic Followups)**: Bổ sung logic trích xuất XML thẻ `<followups>` phía Client [Chatbox.js](file:///d:/Projects/Football_Predict/src/components/chat/Chatbox.js) để tự động sinh 3 nút câu hỏi gợi ý Sentence case cho người dùng.

### Changed (Thay đổi logic)
* **Khắc phục lỗi z-index hiển thị của SweetAlert2**: Sử dụng callback `didOpen` để ép trực tiếp `z-index = 999999` vào phần tử DOM container của Swal, và giảm z-index của khung Chatbox xuống `2000` để đảm bảo modal xác nhận luôn nổi lên trước khung chat.
* **Tối ưu hóa dung lượng token và hiệu năng**: Cắt lát lịch sử tin nhắn chỉ gửi tối đa 12 tin nhắn gần nhất (`messages.slice(-12)`) lên Gemini, tăng 50% tốc độ stream và giảm thiểu tối đa chi phí gọi API.
* **Khắc phục lỗi an toàn dữ liệu sessionStorage**: Thêm helper `safeGetItem` và `safeJsonParse` xử lý và dọn dẹp chuỗi rác hoặc `"undefined"` trong storage của khách, loại bỏ hoàn toàn lỗi crash JS `SyntaxError: "undefined" is not valid JSON`.
* **Tối ưu hóa hiển thị đánh giá chi tiết trong Link Reader**: Định dạng lại chuỗi JSON thô `bet_evaluation_details` sang Markdown tường minh để giúp AI phân tích chính xác tỷ số thực tế và dự đoán.
* **Nâng cấp prompt và cấu hình phản hồi của Trợ lý AI**: Bổ sung chỉ dẫn suy luận đa bước (Chain of Thought) và nguyên tắc kiểm tra đối chiếu dữ liệu nghiêm ngặt để tránh tuyệt đối việc đảo ngược hoặc nhầm lẫn giữa tỷ số thực tế và tỷ số dự đoán.
...
* **Hỗ trợ tự động tạo bảng local**: Tích hợp lệnh tự tạo bảng `users` và `assistant_chats` vào module kết nối DB chính khi khởi chạy SQLite local.
* **Vị trí hiển thị Chatbox nổi**: Điều chỉnh tọa độ Chatbox nổi lên `bottom-24` và di chuyển sang góc bên trái màn hình theo yêu cầu để tránh đè nút điều hành API Activity Float.
* **Responsive Mobile cho Chatbox**: Cấu hình CSS để cửa sổ chat tự động phóng to full-screen trên thiết bị di động, cải thiện diện tích đọc.
...
### Fixed (Sửa lỗi)
* **Khắc phục lỗi thought_signature (400 Bad Request)**: Chỉnh sửa lại cơ chế truyền dữ liệu trong route trợ lý chat [route.js](file:///d:/Projects/Football_Predict/src/app/api/chat/assistant/route.js) để bảo toàn đầy đủ object `functionCall` gốc (bao gồm `id` và `thoughtSignature`) và map chính xác `id` vào phần `functionResponse` cho Gemini, loại bỏ hoàn toàn lỗi `INVALID_ARGUMENT: Function call is missing a thought_signature` khi chat.
* **Tối ưu hóa hiển thị Header trên PC**: Giới hạn chiều rộng tối đa và cấu hình truncate tự động cho `ModelBadge` khi chuỗi tên mô hình AI quá dài, bổ sung `whitespace-nowrap` cho các liên kết điều hướng để chống bẻ dòng chữ và chuẩn hóa chữ menu về Sentence case ("Lịch thi đấu", "Giả lập cặp đấu", "Cấu hình AI").
* **Sửa lỗi thiếu bảng người dùng trên cơ sở dữ liệu Production**: Gộp mã khởi tạo bảng `users` và `assistant_chats` từ các file migration riêng lẻ vào tệp di động chính `scripts/migrate.mjs`, khắc phục triệt để lỗi sập luồng đăng nhập (Mock và Google OAuth) trên Turso DB Production do thiếu bảng.
* **Khắc phục lỗi thiếu nút Đăng nhập / Đăng ký trên di động**: Bổ sung thanh Mobile Header tối giản phía trên cùng hiển thị logo cùng cụm `ModelBadge` và `UserNav`, giúp người dùng trên thiết bị di động có thể đăng nhập, đăng ký và đăng xuất dễ dàng.
* **Thu nhỏ kích thước nút Chatbox AI trên thiết bị di động**: Điều chỉnh kích thước nút bong bóng Chatbox từ `h-14 w-14` cố định sang responsive `h-10 w-10` trên di động để đồng bộ hoàn toàn với kích thước của nút lịch sử thao tác (`ApiActivityFloat`).
...
* **Đồng bộ hóa Prompt hệ thống giữa Local và Prod**: Phát hiện lệch pha nội dung prompt `predict_critic_template` (bản local chi tiết hơn chứa 4 quy tắc logic cược) và thiếu 2 prompt `match_chat_system`, `sync_fixtures_template` ở Local SQLite. Tiến hành đồng bộ và chuẩn hóa đảm bảo 6/6 prompt khớp nhau 100% trên cả Local SQLite và Turso Production DB. Cập nhật mã nguồn mặc định trong `scripts/migrate.mjs` để đồng nhất với prompt mới nhất, chống ghi đè khi chạy migration, đồng thời cấu hình lại `scratch/backup_prompts.mjs` để backup dữ liệu prompt an toàn vào thư mục dự án.gồm `id` và `thoughtSignature`) và map chính xác `id` vào phần `functionResponse` cho Gemini, loại bỏ hoàn toàn lỗi `INVALID_ARGUMENT: Function call is missing a thought_signature` when chat.
* **Tối ưu hóa hiển thị Header trên PC**: Giới hạn chiều rộng tối đa và cấu hình truncate tự động cho `ModelBadge` khi chuỗi tên mô hình AI quá dài, bổ sung `whitespace-nowrap` cho các liên kết điều hướng để chống bẻ dòng chữ và chuẩn hóa chữ menu về Sentence case ("Lịch thi đấu", "Giả lập cặp đấu", "Cấu hình AI").
* **Sửa lỗi thiếu bảng người dùng trên cơ sở dữ liệu Production**: Gộp mã khởi tạo bảng `users` and `assistant_chats` từ các file migration riêng lẻ vào tệp di động chính `scripts/migrate.mjs`, khắc phục triệt để lỗi sập luồng đăng nhập (Mock và Google OAuth) trên Turso DB Production do thiếu bảng.
* **Khắc phục lỗi thiếu nút Đăng nhập / Đăng ký trên di động**: Bổ sung thanh Mobile Header tối giản phía trên cùng hiển thị logo cùng cụm `ModelBadge` và `UserNav`, giúp người dùng trên thiết bị di động có thể đăng nhập, đăng ký và đăng xuất dễ dàng.
* **Thu nhỏ kích thước nút Chatbox AI trên thiết bị di động**: Điều chỉnh kích thước nút bong bóng Chatbox từ `h-14 w-14` cố định sang responsive `h-10 w-10` trên di động để đồng bộ hoàn toàn với kích thước của nút lịch sử thao tác (`ApiActivityFloat`).
* **Khắc phục lỗi không hiển thị bảng so sánh chỉ số của AI**: Sửa đổi phần hiển thị "Lý giải chi tiết" ở cả Match chi tiết và Custom Predictor để gọi hàm parse Markdown dùng chung, khắc phục lỗi bảng bị dính liền dòng.
* **Sửa lỗi hiển thị Responsive của thanh tiến trình Task**: Điều chỉnh CSS/padding của thanh tiến trình nhiệm vụ phù hợp và nhất quán trên cả thiết bị di động (Mobile) lẫn máy tính (PC).
