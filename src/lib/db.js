...
export async function getDB() {
  ...
  // Migrations cho kèo cược động
  try {
    await dbInstance.exec(`ALTER TABLE predictions ADD COLUMN ou_line REAL DEFAULT 2.5`);
  } catch (e) {}
  try {
    await dbInstance.exec(`ALTER TABLE predictions ADD COLUMN corners_line REAL DEFAULT 8.5`);
  } catch (e) {}
  try {
  ...
  // Migrations bổ sung cột provider
  try {
    await dbInstance.exec(`ALTER TABLE api_keys ADD COLUMN provider TEXT DEFAULT 'gemini'`);
  } catch (e) {}
  try {
    await dbInstance.exec(`ALTER TABLE ai_models ADD COLUMN provider TEXT DEFAULT 'gemini'`);
  } catch (e) {}
  ...
  if (modelsCount.count === 0) {
    ...
    // Nếu bảng đã được seed trước đó, ta chèn thêm các model Groq nếu chưa có
    const groqModels = [
    ...
    for (const model of groqModels) {
      const exists = await dbInstance.get(`SELECT COUNT(*) as count FROM ai_models WHERE model_name = ?`, [model.name]);
  ...
  // --- MIGRATION & SEEDING CHO BẢNG TEAMS (QUẢN LÝ ĐỘI TUYỂN) ---
  await dbInstance.exec(`
  ...
  try {
    await dbInstance.exec(`ALTER TABLE teams ADD COLUMN avg_corners_won REAL DEFAULT 4.5`);
  } catch (e) {}
  try {
    await dbInstance.exec(`ALTER TABLE teams ADD COLUMN avg_corners_conceded REAL DEFAULT 4.5`);
  } catch (e) {}
  try {
    await dbInstance.exec(`ALTER TABLE teams ADD COLUMN avg_cards_received REAL DEFAULT 1.8`);
  } catch (e) {}
  try {
  ...
  const promptsCount = await dbInstance.get(`SELECT COUNT(*) as count FROM system_prompts`);
    ...
    const defaultPrompts = [
      {
        ...
        content: `Bạn là một chuyên gia phân tích bóng đá thế giới hàng đầu, chuyên gia soi kèo bóng đá cho kỳ World Cup 2026.
...
Tỷ lệ dự đoán đúng kết quả chung cuộc (1X2) gần đây của bạn với 2 đội này là: {{rate}}% ({{correct}}/{{total}} trận đúng).`
      },
      {
        ...
        content: `Bạn là một Chuyên gia Phản biện Bóng đá và Soi kèo cực kỳ khắt khe. Dưới đây là dự đoán ban đầu từ các mô hình AI khác nhau cho trận đấu giữa {{homeTeam}} và {{awayTeam}}:
...
Lưu ý: Chỉ trả về chuỗi JSON thô, không nằm trong các thẻ code markdown hay ký tự thừa.`
    ...
    console.log('🌱 Seeded default system prompts successfully.');
  }

  // Đảm bảo predict_critic_template được chèn vào kể cả khi bảng đã được seed trước đó
  try {
    const criticPromptExists = await dbInstance.get(`SELECT COUNT(*) as count FROM system_prompts WHERE prompt_key = 'predict_critic_template'`);
    if (!criticPromptExists || criticPromptExists.count === 0) {
      await dbInstance.run(
        `INSERT OR IGNORE INTO system_prompts (prompt_key, prompt_content, description) VALUES (?, ?, ?)`,
        [
          'predict_critic_template',
          `Bạn là một Chuyên gia Phản biện Bóng đá và Soi kèo cực kỳ khắt khe. Dưới đây là dự đoán ban đầu từ các mô hình AI khác nhau cho trận đấu giữa {{homeTeam}} và {{awayTeam}}:
...
Lưu ý: Chỉ trả về chuỗi JSON thô, không nằm trong các thẻ code markdown hay ký tự thừa.`,
          'Mẫu prompt cho Tác nhân Phản biện và Tinh chỉnh dự đoán (Consensus Engine Option 3).'
        ]
      );
      console.log('🌱 Seeded predict_critic_template successfully via separate check.');
    }
  } catch (err) {
    console.error('Lỗi khi chèn bổ sung predict_critic_template:', err);
  }
